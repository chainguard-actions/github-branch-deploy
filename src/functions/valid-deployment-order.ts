...
import type {BranchDeployContext, DeploymentOrderScope} from '../types.ts'

interface ValidDeploymentOrderRequest {
  readonly context: BranchDeployContext
  readonly enforcedDeploymentOrder: readonly string[]
...
export async function validDeploymentOrder({
  ...
  scope,
  sha
}: ValidDeploymentOrderRequest): Promise<{
  ...
  if (scope === 'branch-deploy') {
    core.debug('deployment order scope: branch-deploy')
  }

  if (
    new Set(enforcedDeploymentOrder).size !== enforcedDeploymentOrder.length
  ) {
    throw new Error(
  ...
  const previous_environments = enforcedDeploymentOrder.slice(
    0,
    environmentIndex
  )
  ...
  for (const previous_environment of previous_environments) {
    ...
    if (!is_active) {
      const deploymentLabel =
        scope === 'branch-deploy' ? 'Branch Deploy deployment' : 'deployment'
      core.error(
        `🚦 deployment order checks failed as ${COLORS.highlight}${previous_environment}${COLORS.reset} does not have an active ${deploymentLabel} at sha: ${sha}`
      )
